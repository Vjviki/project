...
class AppStore extends Component {
  state = {
    selectTabItem: tabsList[0].tabId,
    seacrhInput: '',
  }

  tabItemId = tabId => {
    this.setState({selectTabItem: tabId})
  }
  ...
  render() {
    const {selectTabItem} = this.state
    const {seacrhInput} = this.state

    const filterTabList = appsList.filter(
      eachItem => eachItem.category === selectTabItem,
    )

    const searchResult = filterTabList.filter(eachValue =>
      eachValue.appName.toLowerCase().includes(seacrhInput.toLowerCase()),
    )
          ...
          <ul className="tab-item">
            {tabsList.map(eachValue => (
              <TabItem
                ...
                key={eachValue.tabId}
                isActive={eachValue.tabId === selectTabItem}
              />
            ))}
          </ul>
